...
from example_hongniang.items import ExampleHongniangItem

# 刚开始执行时此处犯下一错  继承的父类应该是 CrawlSpider
class HongniangspiderSpider(RedisCrawlSpider):
    name = 'hongniangspider_redis'
    ...
    # 增加 redis key
    redis_key = 'hongniang:start_url'

    # 中国红娘 index 页面的分页
    page_lx = LinkExtractor(allow=(r"index/search\?.*$page=\d+"))
    # 个人详细的信息
    self_lx = LinkExtractor(allow=(r"user/member/id/\d+"))
    # 规则
    rules = (
    ...
    def get_nickname(self, response):
        nickname = response.xpath(
            '//div[@class = "info1"]/div[@class = "name nickname"]/text()').extract()[0]
        if len(nickname) > 0:
    ...
    def get_loveid(self, response):
        ...
        loveid = response.xpath(
            '//div[@class="info1"]/div[@class="loveid"]/text()').extract()[0]
    ...
    def get_photos(self, response):
        ...
        photos = response.xpath(
            '//div[@id="tFocus-btn"]/ul/li/img/@src').extract()
    ...
    def get_age(self, response):
        ...
        age = response.xpath(
            '//div[@class="info2"]/div/ul/li/text()').extract()[0]
    ...
    def get_height(self, response):
        ...
        height = response.xpath(
            '//div[@class="info2"]/div/ul/li/text()').extract()[2]
    ...
    def get_ismarried(self, response):
        ...
        ismarried = response.xpath(
            '//div[@class="info2"]/div/ul/li/text()').extract()[1]
    ...
    def get_yearincome(self, response):
        ...
        yearincome = response.xpath(
            '//div[@class="info2"]/div/ul/li/text()').extract()[4]
    ...
    def get_education(self, response):
        ...
        education = response.xpath(
            '//div[@class="info2"]/div/ul/li/text()').extract()[3]
    ...
    def get_workaddress(self, response):
        ...
        workaddress = response.xpath(
            '//div[@class="info2"]/div/ul/li/text()').extract()[5]
    ...
    def get_soliloquy(self, response):
        ...
        soliloquy = response.xpath(
            '//div[@class="info5"]/div[@class="text"]/text()').extract()[0]
